...
class BDDLBaseDomain(ManipulationEnv):
    ...
    def _load_model(self):
        ...
        if any(v in robot_name for v in _rby1_variants):
            ...
            y_offset = 0.0
        print(f"[DEBUG] y_offset: {y_offset}")

        if self._arena_type == "table":
            xpos = self.robots[0].robot_model.base_xpos_offset["table"](
                self.table_full_size[0]
            )
            print(f"[DEBUG] base_xpos before y_offset: {xpos}")
            xpos = (xpos[0], xpos[1] + y_offset, xpos[2])
            print(f"[DEBUG] base_xpos after y_offset: {xpos}")
